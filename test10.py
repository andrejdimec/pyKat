...
arcpy.mp.CreateWebLayerSDDraft(
    ...
    tags=f"web layer, {layer_name}, ArcGIS Online, arcpy",  # Tags to describe the web layer
    description=f"Automatically created from the '{layer_name}' layer in ArcGIS Pro",  # Detailed description
)
...
try:
    ...
    log(f"Errors:")
    log("Sd not uploaded. Error - {}".format(str(e)))
    delete_temp()
    sys.exit("Napaka pri uploading.")
